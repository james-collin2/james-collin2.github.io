...
class TileMap {
  ...
  draw(s) {
    ...
    } else if (this.order.scheme === "double") {
      drawLayout = drawDoubleLayout;
    ...
    } else if (this.order.scheme === "blends") {
      drawLayout = drawBlendsLayout;
    }
...
const drawRoughLayout = (tileMap, tilesColors, i, j, s, x, y, canDrawTiles) => {
  ...
  if (tileMap.order.schemeType === "ROUGH") {
    if (i % 6 === 0) {
      if ((req + 1) % 4 === 0) {
        if (canDrawTiles) {
          tileMap.tiles.push(new Rectangular(s, x, y, tilesColors["color1"], "color1"));
  ...
  } else if (tileMap.order.schemeType === "ROUGH_X4") {
    if (i % 12 === 0) {
      if ((req + 2) % 8 === 0) {
        if (canDrawTiles) {
          tileMap.tiles.push(new Rectangularx4(s, x, y, tilesColors["color1"], "color1"));
...
const drawDoubleLayout = (tileMap, tilesColors, i, j, s, x, y, canDrawTiles) => {
  ...
  if (tileMap.order.schemeType === "DOUBLE") {
    let reqColor = Math.floor(((j + 1 * i - 1) % 9) / 3);

    if (i === 0) {
      if (req % 12 === 7) {
        if (canDrawTiles) {
          tileMap.tiles.push(new Square(s, x, y - size, tilesColors["color1"], "color1"));
        }
    ...
    if (req % 12 === 0) {
      ...
      if (canDrawTiles) {
        if (tileMap.order.schemeColorType === "4_COLOR") {
          if (reqColor === 0) {
            tileMap.tiles.push(new Dot(s, x, y, tilesColors["color2"], "color2"));
          } else if (reqColor === 1) {
            tileMap.tiles.push(new Dot(s, x, y, tilesColors["color3"], "color3"));
          } else if (reqColor === 2) {
            tileMap.tiles.push(new Dot(s, x, y, tilesColors["color4"], "color4"));
          }
        } else {
          tileMap.tiles.push(new Dot(s, x, y, tilesColors["color2"], "color2"));
        }
      ...
      if (tileMap.order.schemeColorType === "4_COLOR") {
        if (reqColor === 0) {
          ...
          tileMap.order.tilesCount.amountTilesDot[3]++;
        }
      } else {
        tileMap.order.tilesCount.amountTilesDot[1]++;
      }
  ...
  } else if (tileMap.order.schemeType === "DOUBLE_X4") {
    if (i % 2 !== 0) {
      return;
    }

    let reqColor = Math.floor(((Math.floor(j / 2) + 1 * Math.floor(i / 2) - 1) % 9) / 3);

    if (i === 0) {
      if (req % 24 === 14) {
        if (canDrawTiles) {
          tileMap.tiles.push(new Squarex4(s, x, y - size * 2, tilesColors["color1"], "color1"));
        }
    ...
    if (req % 24 === 0) {
      ...
      if (canDrawTiles) {
        if (tileMap.order.schemeColorType === "4_COLOR") {
          if (reqColor === 0) {
            ...
            tileMap.tiles.push(new Square(s, x, y, tilesColors["color4"], "color4"));
          }
        } else {
          tileMap.tiles.push(new Square(s, x, y, tilesColors["color2"], "color2"));
        }
      ...
      if (tileMap.order.schemeColorType === "4_COLOR") {
        if (reqColor === 0) {
          ...
          tileMap.order.tilesCount.amountTilesSquare[3]++;
        }
      } else {
        tileMap.order.tilesCount.amountTilesSquare[1]++;
      }
...
const drawHerringboneLayout = (tileMap, tilesColors, i, j, s, x, y, canDrawTiles) => {
  if (tileMap.order.schemeType === "HERRINGBONE") {
    let req = j + 5 * i;
  ...
  } else if (tileMap.order.schemeType === "HERRINGBONE_X4") {
    let req = j + 5 * i;
...
const drawGridLayout = (tileMap, tilesColors, i, j, s, x, y, canDrawTiles) => {
  ...
  if (tileMap.order.schemeType === "GRID") {
    if (j % 2 === 1) {
      return;
    }

    let reqColor = (j % 3) + 3 * (Math.floor(i / 4) % 2);

    if (i % 4 === 0) {
      if (reqColor === 0 || reqColor === 1) {
        if (canDrawTiles) {
          tileMap.tiles.push(new Rectangular(s, x, y, tilesColors["color1"], "color1", 2));
        }
        ...
        if (canDrawTiles) {
          tileMap.tiles.push(new Rectangular(s, x, y, tilesColors["color4"], "color4", 2));
        }

        tileMap.order.tilesCount.amountTilesRectangular[3]++;
      }
    } else if (i % 4 === 2) {
      if (reqColor === 0 || reqColor === 1) {
        if (canDrawTiles) {
        ...
        if (canDrawTiles) {
          tileMap.tiles.push(new Rectangular(s, x, y, tilesColors["colorInv4"], "colorInv4", 8));
        }
      }
    }
  } else if(tileMap.order.schemeType === "GRID_X4"){
    if (j % 4 !== 0) {
      return;
    }

    let reqColor = ((j/2) % 3) + 3 * (Math.floor(i / 8) % 2);
    ...
    if (i % 8 === 0) {
      if (reqColor === 0 || reqColor === 1) {
        ...
        if (canDrawTiles) {
          tileMap.tiles.push(new Rectangularx4(s, x, y, tilesColors["color4"], "color4", 2));
        }

        tileMap.order.tilesCount.amountTilesRectangularx4[3]++;
      }
    } else if (i % 8 === 4) {
      if (reqColor === 0 || reqColor === 1) {
        if (canDrawTiles) {
        ...
        if (canDrawTiles) {
          tileMap.tiles.push(new Rectangularx4(s, x, y, tilesColors["colorInv4"], "colorInv4", 8));
        }
      }
    }
  }
...
const drawBlendsLayout = (tileMap, tilesColors, i, j, s, x, y, canDrawTiles) => {
  let req = j;
  if(tileMap.order.schemeType === 'BLENDS'){
    if(j%2 !== 0){
      return;
    }
    if(i%2 !== 0){
  ...
  } else if(tileMap.order.schemeType === 'BLENDS_X4'){
    if(j%4 !== 0){
      return;
    }
    if(i%4 !== 0){
