...
const schemes = [
  ...
  },
  {
    ...
    schemeList: [
      ...
      },
      {
        title: "Rough x4",
        type: "ROUGH_X4",
        img: "./assets/img/tiles/rough/roughx4_scheme.png",
        custom: true
      }
    ],
    schemeColorsList: [
  ...
  },
  {
    ...
    schemeColorsList: [
      ...
      },
      {
        title: "four colors",
        type: "4_COLOR",
        img: "./assets/img/tiles/double/double_4color.png"
      },
      {
        title: "glaze",
        type: "GLAZE",
  ...
  },
  {
    ...
    schemeList: [
      ...
      {
        title: "Herringbone x4",
        type: "HERRINGBONE_X4",
        img: "./assets/img/tiles/herringbone/herringbonex4_scheme.png",
        custom: true
      }
    ],
    schemeColorsList: [
...
const layoutSubmit = () => {
  ...
  if (order.schemeColorType !== schemeColor.type) {
    ...
    } else if (schemeColor.type === "3_COLOR") {
      order.colors = [order.colors[0], order.colors[1] || "white", order.colors[2] || "white"];
    } else if (schemeColor.type === "2_COLOR") {
      order.colors = [order.colors[0], order.colors[1] || "white"];
    } else if (
...
let gui = Snap("#tiles-layout");
gui.attr({ viewBox: "0 0 2600 550" });
let paper = Snap("#tiles");
...
const render = state => {
  ...
  if (state === "render") {
    ...
    if (color4Example) {
      color4Example.attr({ fill: colors[order.colors[3]] });
    }
    if (colorFugaExample) {
...
// @title   Render layout
// @called  Before render the tiles we render the upper layout
// @desc    Render the title, colors selector, etc

const renderLayout = (s, xi, yi) => {
  // Info
  ...
  if (order.schemeColorType === "2_COLOR") {
    ...
    let color4 = s.text(1600, 400, "Color 4").attr({
      "font-size": 80
    });
  ...
  let colorFuga = s.text(2150, 400, "Fuga").attr({
    "font-size": 80
  });
  colorFugaExample = s.circle(2450, 375, 70).attr({
    "stroke-width": "1",
    fill: fugaColors[order.fugaColor],
    class: "cursor-pointer",
    stroke: "#888",
  ...
  let dimensionsHeight = s.text(480, 520, `height: ${order.height}cm`).attr({
    "font-size": 50
  });
};
...
const reviewOrder = () => {
  ...
  order.colors.map((val, id) => {
    rows.header += `<td>Color ${id + 1}: <strong>${val}</strong> tiles</td>`;
...
let rotatable = new Draggable(viewport, {
  type: "rotation",
  trigger: svg,
  throwProps: true,
  liveSnap: true,
...
function onWheel(event) {
  ...
  if ((viewBox.width * scaleDelta > 2600 || viewBox.height * scaleDelta > 1400) && delta < 0) {
